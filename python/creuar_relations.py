...
with open('../data/relations_C_church.json', 'r') as f:
...
for esglesia in esglesies_dict:
	...
	overpass_query = "(relation(%s););(._;>;);out;foreach(is_in->.a;area.a[admin_level~'[8]']->.a;convert area::=::,::id =id(),municipi=a.set(t['name']);(._;>;);out;);" % (osm_id)
	...
	for esglesia in myresult:
		...
		if (tecla=='y'):
			...
			mycursor.execute(cadsql)
			print(cadsql)
			cadsql ="UPDATE esglesia set osm_tipus='relation' where id=%s" % (str(id))
			print(cadsql)
			mycursor.execute(cadsql)
			mydb.commit()
			sortir_bucle=1
