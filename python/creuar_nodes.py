...
mysql -u *** -p*** romanic
mysqldump -i --complete-insert -u *** -p*** -r /home/joan/projectes/OSM/esglesies_romaniques/mysql/romanic_200302.sql -v romanic
'''
...
def preparar_consulta(str):
	str = str.lower()
	paraules = ['Mair de Diu ', 'dera ', 'santuari de ', 'capella de ',u'parròquia de ', u'església de ', u'monestir de ', 'sant ', 'santa ', ' de la ', ' dels ', ' de ', ' la ', ' del ', 'ermita ', 'st. ', 'sta. ','st ','sta ', ' d\'', ' l\'']
...
with open('../data/nodes_C_church.json', 'r') as f:
...
for esglesia in esglesies_dict:
	...
	overpass_query = "(node(%i););foreach(is_in->.a;area.a[admin_level~'[8]']->.a;convert node::=::,::id = id(),municipi=a.set(t['name']);out;);" % (osm_id)
	...
	for esglesia in myresult:
		...
		if (tecla=='y'):
			...
			print(cadsql)
			cadsql ="UPDATE esglesia set osm_tipus='node' where id=%s" % (str(id))
			print(cadsql)
			mycursor.execute(cadsql)
			mydb.commit()
			sortir_bucle=1
